...
async def provide_details(request: Request, influencer: InfluencersSchema, db: Session = Depends(get_db), user: User = Depends(get_current_user), status_code = status.HTTP_200_OK):
    # update logged in user details with instagram details.
...
async def search(text: str = None, max_followers: int = None, min_followers: int = None, db: Session = Depends(get_db), status_code = status.HTTP_200_OK):
    ...
    if text:
        return influencers.filter(or_(User.username.contains(text), User.bio.contains(text))).order_by(User.created_at).all()

    if max_followers:
        return influencers.filter(User.followers_count <= max_followers).order_by(User.created_at).all()

    if min_followers:
        return influencers.filter(User.followers_count >= min_followers).order_by(User.created_at).all()
